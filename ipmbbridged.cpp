                                   ...
                                   size_t retriesAttempted = 0)
{
    boost::asio::async_write(
        i2cSlaveSocket, boost::asio::buffer(buffer->data(), buffer->size()),
...
int IpmbChannel::ipmbChannelInit(const char *ipmbI2cSlave)
{
    ...
    // echo "ipmv-dev 0x1010" > /sys/bus/i2c/devices/i2c-0/new_device
    ...
    ipmbi2cSlaveFd = open(ipmbI2cSlave, O_RDWR | O_NONBLOCK);
    if (ipmbi2cSlaveFd < 0)
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
    ...
    i2cSlaveSocket.assign(ipmbi2cSlaveFd);
                            ...
                            std::shared_ptr<IpmbRequest> request)
{
    ...
    for (int i = 0; i < ipmbNumberOfTries; i++)
    {
        ...
        for (; i2cRetryCnt < ipmbI2cNumberOfRetries; i2cRetryCnt++)
        {
            boost::asio::async_write(
                i2cSlaveSocket,
                boost::asio::buffer(buffer.data(), buffer.size()), yield[ec]);
