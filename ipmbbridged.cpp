...
static const uint8_t broadcastSlaveAddress = 0x0;
...
void IpmbChannel::processI2cEvent()
{
        ...
        getChannelType() == ipmbChannelType::ipmb)
    {
        ...
        std::cout << "*** ipmbbridge processI2cEvent(): send signal of "
                                 ...
                                 const uint8_t bmcSlaveAddr)
{
    ...
    if (bmcSlaveAddressChangeable)
    {
        ...
        if (stat(ipmbI2cSlaveFile.c_str(), &sb) < 0)
        {
            ...
            boost::replace_all(deviceFileName, "$busId", std::to_string(busId));
...
int IpmbChannel::ipmbChannelDeinit()
{
    ...
    if (bmcSlaveAddressChangeable)
    {
        ...
        if (stat(ipmbI2cSlaveFile.c_str(), &sb) >= 0)
        {
            std::string deviceFileStr =
                "/sys/bus/i2c/devices/i2c-$busId/delete_device";
            std::string command = "0x$addr";
            boost::replace_all(deviceFileStr, "$busId", std::to_string(busId));
            boost::replace_all(command, "$addr", addressHex.c_str());

            std::fstream deviceFile;
            deviceFile.open(deviceFileStr, std::ios::out);
            if (!deviceFile.good())
            {
                phosphor::logging::log<phosphor::logging::level::ERR>(
                    "ipmbChannelDeinit: error opening deviceFile");
                return -1;
            }
            deviceFile << command;
            deviceFile.close();
...
void addSendBroadcastMsgHandler()
{
    ...
    std::function<void(sdbusplus::message::message &)> sendBroadcastMsgHandler =
        [](sdbusplus::message::message &message) {
            ...
            std::cout
                << "ipmbbridge main(): receive signal of SendBroadcastMessage"
                << " from " << pathName << "\n"
                << " - ipmiCmd: 0x" << std::hex << static_cast<uint16_t>(cmd)
                << "\n";
