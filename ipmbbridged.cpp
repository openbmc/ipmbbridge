...
{
    std::fstream deviceFile;
    std::string deviceFileName = "/sys/bus/i2c/devices/i2c-" + std::to_string(ipmbBusId) +
                     "/delete_device";
    std::string para = "0x1010"; // align with removed ipmb0 definition in dts file
    deviceFile.open(deviceFileName, std::ios::out);
    if (!deviceFile.good())
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
...
std::optional<std::tuple<int, uint8_t, uint8_t, uint8_t, uint8_t, std::vector<uint8_t>>> IpmbChannel::requestAddRetry(std::shared_ptr<IpmbRequest> request, boost::asio::yield_context& yield, std::vector<uint8_t>& buffer)
{
