...
{
    // if target addr is same as one when init ipmbi2cMasterFd, write directly
    if ((*buffer)[0] == ipmbRqSlaveAddress)
    {
                ...
                if (ec)
                {
                    ...
                    if (currentRetryCnt > ipmbI2cNumberOfRetries)
                    {
                        phosphor::logging::log<phosphor::logging::level::ERR>(
                            "ipmbSendI2cFrame: sent to I2C failed after "
                            "retries");
                        return;
                    }
                    currentRetryCnt++;
    ...
    // if target addr is different, need to construct i2c frame and call ioctl
    else
    {
        io.post([this, buffer, retriesAttempted]() {
...
void IpmbChannel::processI2cEvent()
{
    ...
    if (ipmbFrame->Header.Req.address == 0x0 &&
    ...
    else
    {
        ...
        if (commandFilter)
        {
            ...
            uint8_t addr = ipmbFrame->Header.Req.rqSA;
...
int IpmbChannel::ipmbChannelUpdateSlaveAddress(const uint8_t newBmcSlaveAddr)
{
    ...
    deviceFileName = "/sys/bus/i2c/devices/i2c-" + std::to_string(ipmbBusId) +
                     "/delete_device";
    para = "0x1010"; // assume old BMC addr is 0x20
    deviceFile.open(deviceFileName, std::ios::out);
    if (!deviceFile.good())
    {
        phosphor::logging::log<phosphor::logging::level::ERR>(
...
void addSendBroadcastHandler()
{
    ...
    std::function<void(sdbusplus::message::message &)> sendBroadcastHandler =
        [](sdbusplus::message::message &message) {
            ...
            uint8_t targetAddr = 0x0; // 0x0 is defined for broadcast msg
